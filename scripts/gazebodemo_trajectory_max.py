...
class Generator:
    ...
    def __init__(self):
        ...
        self.thetaA = np.zeros((self.N, 1))
        self.thetaB = np.array([-np.pi/2, np.pi/4, 0.0, -np.pi/2, 0.0, -np.pi/4, 0.0]).reshape((-1,1))
        ...
        #self.segments = (Hold(self.thetaA, 2.0),
        #		         Goto(self.thetaA, self.thetaB, 2.0),
        #		         Stay(self.thetaB))
    ...
    def update(self, t, dt):
        ...
        if (t <= 2):
        	...
        	theta_diff = (1/2) * np.subtract(self.thetaB, self.thetaA)
        	theta = self.thetaA + (t - 2) * theta_diff
        else:
        	theta = self.thetaB
